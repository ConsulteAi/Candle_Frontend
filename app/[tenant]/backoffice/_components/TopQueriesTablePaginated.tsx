...
export function TopQueriesTablePaginated() {
  ...
  const { data, isLoading } = useSWR(
    'admin-query-types-all',
    () =>
      getQueryTypesAction({ limit: 200, page: 1 }).then((res) => {
  ...
  // Global top-3 by revenue — medals always reflect global rank
  ...
  const filtered = useMemo(() => {
    ...
    return enriched.filter(
      (e: Enriched) => e.name.toLowerCase().includes(q) || e.code.toLowerCase().includes(q),
    );
  }, [enriched, search]);
  ...
  return (
    <Card className="shadow-glass border-none overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="flex-1 min-w-0 space-y-0.5">
            ...
            <CardDescription className="text-xs">
              ...
              {data && (
                <span className="text-slate-400 ml-1">— {data.total} produtos</span>
              )}
            </CardDescription>
          </div>
      ...
      <CardContent className="p-0">
        <div
          className={`overflow-x-auto transition-opacity duration-200 ${isLoading ? 'opacity-40' : 'opacity-100'}`}
        >
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {pageItems.map((query) => {
                const globalRankIdx = top3Ids.indexOf(query.id);
                const medal = globalRankIdx >= 0 ? MEDAL_CONFIG[globalRankIdx] : null;
                const globalRank = sorted.findIndex((q) => q.id === query.id) + 1;
