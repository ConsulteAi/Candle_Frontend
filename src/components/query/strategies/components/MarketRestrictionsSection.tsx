...
import { AlertTriangle, Calendar, CheckCircle2, FileWarning, Landmark } from 'lucide-react';
import type {
  RaioXGenericDebt,
...
/**
 * Total de pendências financeiras do bloco, na mesma regra do PDF
 * (`countFinancialDebts` em `financial-sections`/`extra-sections` no backend):
 * pendências genéricas chegam em `debts` sem estar refletidas nos contadores por
 * base, então o total é o maior entre o informado pelo provider e a lista
 * efetivamente renderizada. Registros `nadaConsta` são placeholders de "sem
 * ocorrência" e não contam.
 */
export function countFinancialDebts(
  summary: RaioXMarketRestrictionsSummary | undefined,
  debts: RaioXGenericDebt[] | undefined,
): number {
  const listedDebts = (debts ?? []).filter((debt) => !debt?.nadaConsta).length;
  return Math.max(Number(summary?.totalDebts || 0), listedDebts);
}
...
export function MarketRestrictionsSection({
  ...
  const totalDebts = countFinancialDebts(summary, mr.debts);
    ...
    <div className="space-y-5 p-4">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
        <SummaryCard
          title="Pendências"
          value={totalDebts}
          subtitle={totalDebts > 0 ? 'Total de pendências' : 'Nada consta'}
